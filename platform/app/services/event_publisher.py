...
from rdh_contract.events import EVENT_MODELS, EVENT_REGISTRY, EventSpec, get_spec
...
class PreparedEvent(NamedTuple):
    """校验通过、可投递的事件。"""
...
def prepare_event(routing_key: str, payload: ContractModel) -> PreparedEvent:
    """校验并构造事件信封。

    校验两件事：routing_key 已注册，payload 类型与注册表声明一致。错发的事件在发布方
    就被拦住，而不是让 Scheduler 反序列化失败。

    两个后端共用这一份信封格式 —— Scheduler 按同一结构解析文件队列与 RabbitMQ，
    格式漂移会让切后端时静默失败。
    """
    ...
    return PreparedEvent(event_id=event_id, envelope=envelope, spec=spec)
...
class FileQueuePublisher:
    ...
    async def publish(self, routing_key: str, payload: ContractModel) -> str:
        """校验并投递事件。按 ``consumer_queue`` 分目录 —— 等价于 topic exchange 的绑定。"""
