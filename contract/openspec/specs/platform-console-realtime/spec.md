# platform-console-realtime Specification

## Purpose
定义 Platform 向浏览器控制台推送实时状态的契约：Agent 上下线与上传进度。
与 Agent 的 WebSocket 是两条独立通道（`console.` 帧 vs `down.` 帧），
推送为单向，所有操作仍走 REST。推送不可用时轮询兜底，因此断线是降级而非故障。
