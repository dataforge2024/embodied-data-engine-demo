...
需求是「把所有状态流转可视化出来」。现状是**查不到历史**：

```
apply_transition (repositories/episode.py:114)
    row.status = target.value    ← 直接覆盖，旧状态就此消失
    row.updated_at = now()       ← 只知道「最后改过」，不知道改了什么、谁改的

平台的五张表：episodes / annotations / collect_tasks / agent_nodes / users
             ✗ 没有流转历史表（grep audit / history / transition_log 零命中）
```

这个缺失已经在代码里留下痕迹。`utils/stage.ts:78`：

> 脱轨：卡在哪一格取决于死之前走到哪，但 Episode 只存当前状态，拿不到历史。
> 保守起见把所有阶段标 blocked，由调用方单独渲染脱轨态。

所以失败的 Episode 在进度条上是一整条灰的 —— 看不出它死在采集还是标注。**加流转记录顺带
把这个短板修掉。**

### 记录点放在哪

`apply_transition` 是状态写入的**唯一入口**，注释里明确写了「唯一合法调用方是
`services/episode_lifecycle.py`」，且 conformance 测试守着这条纪律
（`test_module_conformance.py` 里检查只有 lifecycle 调它）。

| 方案 | 记录点 | 漏记风险 |
|---|---|---|
| 甲 各业务方法自己记 | `mark_uploaded` / `reject` / `publish_episode` … | 高 —— 新增路径时容易忘 |
| **乙 收口在 apply_transition** | 一处 | 无 —— 状态变不了而不经过它 |
| 丙 DB 触发器 | 数据库层 | 拿不到「谁触发的」，那是应用层信息 |

**选乙。** 一处记录点，且它已经被架构纪律保护着。代价是 `apply_transition` 需要额外知道
「谁触发的」—— 这个参数得从 lifecycle 传进来。

### 触发者怎么表达

轨迹要能回答「谁在什么时候推的」，而推进者有两类：

```
人工   质检 / 标注 / 审核 / 导出        → 记 user_id
系统   上传回调后进 processing           → 记「系统 + 环节名」
       算子回调后进 verification_pending
       送标完成后进 annotation_pending
```

不把系统推进伪装成某个用户（比如记成 admin）—— 那会让轨迹里出现管理员从没做过的操作。

### 重放不记

`episode_lifecycle` 对重放的处理是「目标状态已达成则返回现状，`changed=False`」。这种情况
**不追加记录** —— 状态没变，记一条会让轨迹里出现假的停顿，而幂等重放是 RabbitMQ 至少一次
投递下的常态，记录会被噪音淹没。

### 停留时长

轨迹按时间正序返回，相邻两条记录的时间差就是前一个状态的停留时长。不额外存这个字段 ——
它是可推导的，存了就有不一致的风险。

「卡在哪一步」这个问题靠它回答：某个状态停留时间显著长于其他，说明那一环是瓶颈。
...
**流转记录只覆盖 Episode 状态。** 任务状态、标注修订版本的变更不记 —— 它们各有自己的
字段（`revision`、`collected_count`），本变更不把它们统一进流转历史。

**记录只增不删。** POC 阶段不做清理策略。真实场景下每条 Episode 约 10 条记录，量不大，
但没有归档机制这件事要知道。
...
- 流转记录的聚合视图（跨 Episode 看各环节平均耗时、瓶颈环节）—— 本变更只做单条轨迹
